...
discoveredOptions: dict[str, bool] = {'guard change': False, 'club': False, 'dagger': False, 'lock picks': False, 'key wax': False}
...
class SmugglersHideout:
    def describe_main_room(returning: Optional[bool]):
        ...
        match (decide(["inspect map", 'approach smugglers', 'enter back','leave hideout'], True)):
            ...
            case 2:
                SmugglersHideout.describe_back_hall(False)
            case 3:
    ...
    def describe_back_hall(returning: Optional[bool]):
        ...
        match decision:
            case 0:
                SmugglersHideout.describe_weapons_room(discoveredOptions['dagger'] or discoveredOptions['club'])
            case 1:
            ...
            case 3:
                SmugglersHideout.describe_main_room(True)
    def describe_weapons_room(returning: Optional[bool]):
        if returning:
            print("You are back in the closet")
        else:
    ...
    def inspect_weapons_in_storage():
        ...
        if discoveredOptions['club']:
            print('While the club fits nicely in your hand, you could switch it out with the dagger if you wanted, or drop the \n'
                  'club back here. After all, you\'ve never needed a weapon before.')
            decision = decide(['switch with dagger','leave club','go back'],False)
            match decision:
                case 0:
                    print("The dagger falls into your hand heavily, though you are reluctant to leave the club here, you \n"
                          "know you'll need at least one hand.")
                    discoveredOptions['dagger'] = True
                    discoveredOptions['club'] = False
                    SmugglersHideout.describe_weapons_room(True)
                case 1:
                    print("You've never needed a weapon before, so why should you need one now? You leave the club on the shelf.")
                    discoveredOptions['club'] = False
                    SmugglersHideout.describe_weapons_room(True)
                case 2:
                    SmugglersHideout.describe_weapons_room(True)
        elif discoveredOptions['dagger']:
            print('While you already have a dagger, you know how to use the club better, and it can prove more useful in \n'
            ...
            print("A dozen or so weapons hang on a wooden rack that seems to have been jerry rigged from scraps of rotting wood. \n"
            ...
            match decision:
                case 0:
                    print("The club falls heavily into your hand, and offers a reassuring presence, though you hope you won't need it.")
                    discoveredOptions['club'] = True
                    SmugglersHideout.describe_weapons_room(True)
                case 1:
                    print("The dagger feels heavy in your hand, though you aren't sure, if push came to shove, you would be able to use it.")
                    discoveredOptions['dagger'] = True
                    SmugglersHideout.describe_weapons_room(True)
                case 2:
                    SmugglersHideout.describe_weapons_room(True)
    def inspect_tools_in_storage():
        ...
        if discoveredOptions['lock picks']:
            ...
            decision = decide(['take key wax', 'leave lock picks', 'go back'], False)
            match decision:
                case 0:
            ...
            match decision:
                case 0:
                    if discoveredOptions['lock picks']:
                        ...
                        print(
                            "You slip the lock picks into your pocket, it's better to be safe than sorry.")
